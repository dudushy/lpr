...
output_folder_path = path + r"\assets\output"  # ? Assets output folder path
model_file_path = path + r"\model\binary_128_0.50_ver3.pb"  # ? Model file path
label_file_path = path + r"\model\binary_128_0.50_labels_ver2.txt"  # ? Label file path
...
def test() -> None:
    ...
    print(f"[{title}#test] output_folder_path: ", output_folder_path)
    print(f"[{title}#test] model_file_path: ", model_file_path)
    print(f"[{title}#test] label_file_path: ", label_file_path)
    print(f"[{title}#test] tf.__version__: ", tf.__version__)
...
def lpr() -> None:
    ...
    for file in files:
        try:
            ...
            possible_plates = findPlate.find_possible_plates(file)
